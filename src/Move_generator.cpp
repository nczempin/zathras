...
//      << endl;
  ...
  moves.clear();
  function<void(int8_t, uint8_t, uint8_t, int8_t)> f =
      [&moves, this, square](const int8_t moving, const uint8_t from, const uint8_t to, const int8_t captured) {
        bool en_passant = false;
        if ((moving == Piece::WHITE_PAWN || moving == Piece::BLACK_PAWN) &&Position::is_set_square(p->en_passant_square, to)) {
          en_passant = true;
        }
...
//   attack_moves.clear();
  function<void(int8_t, uint8_t, uint8_t, int8_t)> f =
      [this, square](const int8_t moving, const uint8_t from, const uint8_t to, const int8_t captured) {
//        bool en_passant = false;
//        if ((moving == Piece::WHITE_PAWN || moving == Piece::BLACK_PAWN) &&Position::is_set_square(p->en_passant_square, to)) {
//          en_passant = true;
//        }
