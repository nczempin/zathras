...
void Move_generator::print_moves_raw(const bb sub_position,
	...
	visit_moves_raw(sub_position, all_moves,
			[](int8_t pc, uint8_t x, uint8_t y, int8_t cpt, int8_t promoted_tovisit_pawn_caps) {
...
void Move_generator::add_non_capture_ray_moves(Move_container& moves,
	...
	while (position != 0) {
		const uint8_t from = Bitboard::extract_and_remove_square(position);
		const bb raw_moves = pieceMoves[from];
...
//moves.reserve(35);
	...
	const move_visitor &f =
			[&moves, this](const int8_t &moving, const uint8_t &from, const uint8_t &to,
					const int8_t &captured,const int8_t& promoted_to) {
...
//TODO generalize, obviously
	...
	if (p->white_to_move) {
		visit_capture_moves(white_knights, Bitboard::knight_moves, f, p->black,
				Piece::WHITE_KNIGHT);
		visit_non_capture_moves(white_knights, Bitboard::knight_moves, f, occupied,
				Piece::WHITE_KNIGHT);
		visit_capture_moves(white_kings, Bitboard::king_moves, f, p->black,
				Piece::WHITE_KING);
		visit_non_capture_moves(white_kings, Bitboard::king_moves, f, occupied,
				Piece::WHITE_KING);
		add_non_capture_ray_moves(moves, Piece::WHITE_QUEEN, white_queens,
				Bitboard::rook_moves, occupied);
		add_non_capture_ray_moves(moves, Piece::WHITE_ROOK, white_rooks,
				Bitboard::rook_moves, occupied);
		...
		visit_capture_ray_moves(white_rooks, Bitboard::rook_moves, f, occupied, p->black,
				Piece::WHITE_ROOK);
		visit_capture_ray_moves(white_bishops, Bitboard::bishop_moves, f, occupied,
				p->black, Piece::WHITE_BISHOP);
		visit_capture_ray_moves(white_queens, Bitboard::bishop_moves, f, occupied,
				p->black, Piece::WHITE_QUEEN);
		add_non_capture_ray_moves(moves, Piece::Piece::WHITE_BISHOP,
				white_bishops, Bitboard::bishop_moves, occupied);
		add_non_capture_ray_moves(moves, Piece::WHITE_QUEEN, white_queens,
				Bitboard::bishop_moves, occupied);
		generate_castling(f, true);
		visit_pawn_caps(white_pawns, Bitboard::white_pawn_capture_moves, f, p->black,
				Piece::WHITE_PAWN);
		visit_pawn_nocaps(white_pawns, Bitboard::white_pawn_no_capture_moves, f, occupied,
				Piece::WHITE_PAWN, true);
	} else {
		visit_capture_moves(black_knights, Bitboard::knight_moves, f, p->white,
				Piece::BLACK_KNIGHT);
		visit_non_capture_moves(black_knights, Bitboard::knight_moves, f, occupied,
				Piece::BLACK_KNIGHT);
		visit_capture_moves(black_kings, Bitboard::king_moves, f, p->white,
				Piece::BLACK_KING);
		visit_non_capture_moves(black_kings, Bitboard::king_moves, f, occupied,
				Piece::BLACK_KING);
		add_non_capture_ray_moves(moves, Piece::BLACK_QUEEN, black_queens,
				Bitboard::rook_moves, occupied);
		add_non_capture_ray_moves(moves, Piece::BLACK_ROOK, black_rooks,
				Bitboard::rook_moves, occupied);
		...
		visit_capture_ray_moves(black_rooks, Bitboard::rook_moves, f, occupied, p->white,
				Piece::BLACK_ROOK);
		visit_capture_ray_moves(black_bishops, Bitboard::bishop_moves, f, occupied,
				p->white, Piece::BLACK_BISHOP);
		visit_capture_ray_moves(black_queens, Bitboard::bishop_moves, f, occupied,
				p->white, Piece::BLACK_QUEEN);
		add_non_capture_ray_moves(moves, Piece::Piece::BLACK_BISHOP,
				black_bishops, Bitboard::bishop_moves, occupied);
		add_non_capture_ray_moves(moves, Piece::BLACK_QUEEN, black_queens,
				Bitboard::bishop_moves, occupied);

		generate_castling(f, false);
		visit_pawn_caps(black_pawns, Bitboard::black_pawn_capture_moves, f, p->white,
				Piece::BLACK_PAWN);
		visit_pawn_nocaps(black_pawns, Bitboard::black_pawn_no_capture_moves, f, occupied,
